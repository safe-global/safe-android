...
class SafeMainViewModel @Inject constructor(
    ...
    // Fuck you kotlin why do I have to add this map for type casting?
        safeRepository.loadSafe(addressOrHash).map<AbstractSafe> { it }
            .onErrorResumeNext {
                safeRepository.loadPendingSafe(addressOrHash)
            }

    override fun selectSafe(address: BigInteger): Single<out AbstractSafe> =
        Single.fromCallable {
            preferenceManager.prefs.edit { putString(KEY_SELECTED_SAFE, address.toHexString()) }
        }.flatMap {
            loadSelectedSafe()
        }
