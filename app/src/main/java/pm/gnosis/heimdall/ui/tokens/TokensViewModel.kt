...
class TokensViewModel @Inject constructor(@ApplicationContext private val context: Context,
    ...
    override fun observeTokens(refreshEvents: Observable<Unit>, ofAddress: BigInteger?): Observable<out Result<Adapter.Data<ERC20TokenWithBalance>>> =
            Observable
                    ...
                    .flatMap { tokens ->
                        if (ofAddress != null) {
                            loadTokenBalances(ofAddress, tokens)
                        } else {
    ...
    private fun loadTokenBalances(ofAddress: BigInteger, tokens: List<ERC20Token>) =
            tokenRepository.loadTokensBalance(ofAddress, tokens)
                    .map {
