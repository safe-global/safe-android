...
class MnemonicKeyAndAddressDerivatorTest {
    ...
    val keyAndAddressDerivator: MnemonicKeyAndAddressDerivator = MnemonicKeyAndAddressDerivator(mockBip39)

    @Before
    fun setUp() {
        keyAndAddressDerivator.initialize("creek banner employ mix teach sunny sure mutual pole mom either lion")
    }

    @Test
    fun `addressesForRange (0-0) should return first address`() {
        val firstAddress = keyAndAddressDerivator.addressesForRange(0L..0L)
    ...
    fun `addressesForRange (0-2) should return first three addresses`() {
        val firstAddress = keyAndAddressDerivator.addressesForRange(0L..2L)
    ...
    fun `addressesForRange (2-2) should return third address`() {
        val firstAddress = keyAndAddressDerivator.addressesForRange(2L..2L)
    ...
    fun `addressesForPage (PageSize 1) should return first address`() {
        ...
        val firstAddress = keyAndAddressDerivator.addressesForPage(0, 1)
    ...
    fun `addressesForPage (PageSize 0) should return empty list`() {
        ...
        val firstAddress = keyAndAddressDerivator.addressesForPage(0, 0)
    ...
    fun `keyForIndex (inde 0) should return correct private key`() {
        val secretKey = keyAndAddressDerivator.keyForIndex(0L)
